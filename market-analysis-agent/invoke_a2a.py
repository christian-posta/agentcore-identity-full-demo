#!/usr/bin/env python3
"""
Invoke the deployed A2A market analysis agent via message/send (SigV4).
Reads agent ARN (and region) from .bedrock_agentcore.yaml in this directory when run
from market-analysis-agent; override with AGENT_ARN / AWS_REGION env vars if needed.
"""
import argparse
import json
import os
...
def _get_agent_arn() -> str:
    ...
    if env_arn:
        return env_arn
    return "arn:aws:bedrock-agentcore:us-west-2:<account>:runtime/a2a_market_analysis_agent-<id>"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Invoke A2A market analysis agent (message/send, SigV4)")
    parser.add_argument(
        "message",
        nargs="?",
        default="Analyze MacBook Pro inventory for Q2 onboarding of 50 new engineers",
        help="User message to send (default: Analyze MacBook Pro inventory for Q2 onboarding of 50 new engineers)",
    )
